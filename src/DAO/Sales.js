...
await salesinho.run('CREATE TABLE IF NOT EXISTS Sales (id INTEGER PRIMARY KEY NOT NULL, client VARCHAR(45), salesman VARCHAR(45), campaign VARCHAR(55), budget NUMBER(4,2), total NUMBER(4,2), date DATE, form_of_payment VARCHAR(45))')
...
export async function insertSales() {
  let Sales2 = req.body
  salesinho.run('INSERT INTO Sales(client, salesman, campaign, budget, total, date, form_of_payment) VALUES (?,?,?,?,?,?,?)', [Sales2.client, Sales2.salesman, Sales2.campaign, Sales2.budget, Sales2.total, Sales2.date, Sales2.form_of_payment])
};

//Atualização de uma tabela existente
export async function selectSales2() {
  return salesinho.all('SELECT * FROM Sales')
};
